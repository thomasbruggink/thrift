...
void t_kotlin_generator::print_const_value(std::ostream &out,
  ...
  if (type->is_base_type()) {
    ...
    if (is_enum_set(type)) {
      out << name << " = " << type_name(type, false, true, true) << ".noneOf(" << inner_enum_type_name(type) << ");"
          << endl;
    } else {
      out << name << " = new " << type_name(type, false, true) << "();" << endl;
    }
    ...
    switch (tbase) {
      case t_base_type::TYPE_STRING:
        if (((t_base_type *) type)->is_binary()) {
          render << "kotlin.nio.ByteBuffer.wrap(\"" << get_escaped_string(value) << "\".getBytes())";
        } else {
...
void t_kotlin_generator::generate_union_getters_and_setters(ostream &out, t_struct *tstruct) {
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    if (type->is_binary()) {
      ...
      indent(out) << "    throw new kotlin.lang.RuntimeException(\"Cannot get field '" << field->get_name()
...
void t_kotlin_generator::generate_check_type(ostream &out, t_struct *tstruct) {
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    indent(out) << "  }" << endl;
    indent(out) << "  throw new kotlin.lang.ClassCastException(\"Was expecting value of type "
  ...
  indent(out) << "  throw new kotlin.lang.IllegalArgumentException(\"Unknown field id \" + setField);" << endl;
...
void t_kotlin_generator::generate_standard_scheme_read_value(ostream &out, t_struct *tstruct) {
  ...
  indent(out) << "default:" << endl;
  indent(out) << "  throw new kotlin.lang.IllegalStateException(\"setField wasn't null, but didn't match any "
...
void t_kotlin_generator::generate_standard_scheme_write_value(ostream &out, t_struct *tstruct) {
  ...
  indent(out) << "default:" << endl;
  indent(out) << "  throw new kotlin.lang.IllegalStateException(\"Cannot write union with unknown field \" + "
                 "setField_);" << endl;
...
void t_kotlin_generator::generate_kotlin_struct_definition(ostream &out,
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    t_field *field = (*m_iter);
    std::string field_name = decapitalize(field->get_name());
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    if (t->is_struct()) {
      ...
      if (bt->is_binary()) {
        ...
        indent_up();
        indent(out) << prefix << " = kotlin.nio.ByteBuffer.wrap(in.createByteArray());" << endl;
        scope_down(out);
      } else {
        switch (bt->get_base()) {
          case t_base_type::TYPE_I16:
...
void t_kotlin_generator::generate_kotlin_struct_equality(ostream &out, t_struct *tstruct) {
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    string name = decapitalize((*m_iter)->get_name());
...
void t_kotlin_generator::generate_kotlin_struct_compare_to(ostream &out, t_struct *tstruct) {
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    t_field *field = *m_iter;
    string field_name = decapitalize(field->get_name());
...
void t_kotlin_generator::generate_kotlin_struct_reader(ostream &out, t_struct *tstruct) {
  (void) tstruct;
  indent(out) << "override fun read(iprot: org.apache.thrift.protocol.TProtocol) {" << endl;
  indent_up();
...
void t_kotlin_generator::generate_kotlin_struct_writer(ostream &out, t_struct *tstruct) {
  (void) tstruct;
  indent(out) << "override fun write(oprot: org.apache.thrift.protocol.TProtocol) {" << endl;
  indent_up();
...
void t_kotlin_generator::generate_kotlin_struct_result_writer(ostream &out, t_struct *tstruct) {
  (void) tstruct;
  indent(out) << "fun write(oprot: org.apache.thrift.protocol.TProtocol) {" << endl;
  indent_up();
  indent(out) << "scheme<" << tstruct->get_name() << ">(oprot).write(oprot, this)" << endl;
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    if (type->is_set() || type->is_list()) {
      ...
      if (optional) {
        ...
        indent(out) << "public org.apache.thrift.Option<kotlin.util.Iterator<" << type_name(element_type, true, false)
                    << ">> get" << cap_name;
        ...
        indent(out) << kotlin_nullable_annotation() << endl;
        indent(out) << "public kotlin.util.Iterator<" << type_name(element_type, true, false)
                    << "> get" << cap_name;
        out << get_cap_name("iterator() {") << endl;
...
void t_kotlin_generator::generate_service_async_client(t_service *tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    string kotlinname = funname;
    sep = "";
    kotlinname = as_camel_case(kotlinname);
    ...
    f_service_
            ...
            << endl << indent() << "  throw new kotlin.lang.IllegalStateException(\"Method call not finished!\");"
    ...
    if (ret_type->is_void()) { // NB: Includes oneways which always return void.
      ...
      f_service_ << "return (new Client(prot)).recv" + sep + kotlinname + "();" << endl;
    }
...
void t_kotlin_generator::generate_service_server(t_service *tservice) {
  ...
  indent(f_service_) << "  super(iface, getProcessMap(new kotlin.util.HashMap<kotlin.lang.String, "
...
void t_kotlin_generator::generate_service_async_server(t_service *tservice) {
  ...
  indent(f_service_) << "protected AsyncProcessor(I iface, kotlin.util.Map<kotlin.lang.String,  "
                        "org.apache.thrift.AsyncProcessFunction<I, ? extends  "
                        "org.apache.thrift.TBase, ?>> processMap) {" << endl;
  indent(f_service_) << "  super(iface, getProcessMap(processMap));" << endl;
  indent(f_service_) << "}" << endl << endl;

  indent(f_service_) << "private static <I extends AsyncIface> kotlin.util.Map<kotlin.lang.String,  "
                        "org.apache.thrift.AsyncProcessFunction<I, ? extends  "
                        "org.apache.thrift.TBase,?>> getProcessMap(kotlin.util.Map<kotlin.lang.String,  "
                        "org.apache.thrift.AsyncProcessFunction<I, ? extends  "
                        "org.apache.thrift.TBase, ?>> processMap) {" << endl;
  indent_up();
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
 ...
 */
void t_kotlin_generator::generate_process_async_function(t_service *tservice, t_function *tfunction) {
  ...
  if (!tfunction->is_oneway()) {
    ...
    indent_down();
    indent(f_service_) << "} catch (kotlin.lang.Exception e) {" << endl;
    indent_up();
  ...
  indent(f_service_) << "public void onError(kotlin.lang.Exception e) {" << endl;
  indent_up();
  ...
  if (tfunction->is_oneway()) {
    ...
    f_service_ << indent() << "}" << endl
               ...
               << indent() << "} catch (kotlin.lang.Exception ex) {" << endl
 ...
 * @param tfunction The function to write a dispatcher for
 */
void t_kotlin_generator::generate_process_function(t_service *tservice, t_function *tfunction) {
  string argsname = tfunction->get_name() + "_args";
  string resultname = tfunction->get_name() + "_result";
  if (tfunction->is_oneway()) {
    resultname = "org.apache.thrift.TBase";
...
void t_kotlin_generator::generate_deserialize_field(ostream &out,
  ...
  string name = prefix + decapitalize(tfield->get_name());
...
void t_kotlin_generator::generate_deserialize_map_element(ostream &out,
  ...
  indent(out) << declare_field(&fkey, false) << endl;
  indent(out) << declare_field(&fval, false) << endl;
...
void t_kotlin_generator::generate_deserialize_set_element(ostream &out,
  ...
  indent(out) << declare_field(&felem, false) << endl;
...
void t_kotlin_generator::generate_serialize_container(ostream &out,
  ...
  if (ttype->is_map()) {
    indent(out) << "for (kotlin.util.Map.Entry<" << type_name(((t_map *) ttype)->get_key_type(), true, false)
 ...
 * @return kotlin type name, i.e. kotlin.util.HashMap<Key,Value>
 */
string t_kotlin_generator::type_name(t_type *ttype,
  ...
  if (ttype->is_base_type()) {
    ...
    if (in_init) {
      if (is_enum_map(tmap)) {
        prefix = "kotlin.util.EnumMap";
      } else {
        prefix = "kotlin.util.HashMap";
      }
    } else {
      prefix = "kotlin.util.Map";
    }
    ...
    if (in_init) {
      if (is_enum_set(tset)) {
        prefix = "kotlin.util.EnumSet";
      } else {
        prefix = "kotlin.util.HashSet";
      }
    } else {
      prefix = "kotlin.util.Set";
    }
    return prefix + (skip_generic ? "" : "<" + type_name(tset->get_elem_type(), true) + ">");
  } else if (ttype->is_list()) {
    t_list *tlist = (t_list *) ttype;
    if (in_init) {
      prefix = "kotlin.util.ArrayList";
    } else {
      prefix = "kotlin.util.List";
    }
...
string t_kotlin_generator::declare_field(t_field *tfield, bool comment, bool last) {
  ...
  result += decapitalize(tfield->get_name()) + ": " + type_name(tfield->get_type());
  if (type_can_be_null(ttype)) {
    result += kotlin_nullable_annotation() + " ";
  }
...
void t_kotlin_generator::generate_deep_copy_container(ostream &out,
  ...
  if (container->is_map()) {
    ...
    indent(out) << "for (kotlin.util.Map.Entry<" << type_name(key_type, true, false) << ", "
    ...
    if (elem_type->is_container()) {
      ...
      if (elem_type->is_binary()) {
        indent(out) << "kotlin.nio.ByteBuffer temp_binary_element = ";
        generate_deep_copy_non_container(out,
...
void t_kotlin_generator::generate_kotlin_struct_read_object(ostream &out, t_struct *tstruct) {
  indent(out) << "fun readObject(inp:java.io.ObjectInputStream) {" << endl;
  indent(out) << "  try {" << endl;
  if (!tstruct->is_union()) {
    switch (needs_isset(tstruct)) {
      case ISSET_NONE:
        break;
      case ISSET_PRIMITIVE:
        indent(out) << "    // it doesn't seem like you should have to do this, but kotlin "
...
void t_kotlin_generator::generate_standard_reader(ostream &out, t_struct *tstruct) {
  out << indent() << "override fun read(iprot:org.apache.thrift.protocol.TProtocol, "
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    indent(out) << "struct."
                << "set" << (*f_iter)->get_name() << get_cap_name("isSet")
                << "(true);" << endl;
    indent_down();
...
void t_kotlin_generator::generate_standard_writer(ostream &out, t_struct *tstruct, bool is_result) {
  indent_up();
  out << indent() << "override fun write(oprot:org.apache.thrift.protocol.TProtocol, "
      << "struct:" << tstruct->get_name() << ") {" << endl;
  indent_up();
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    if (null_allowed) {
      out << indent() << "if (struct." << decapitalize((*f_iter)->get_name()) << " != null) {" << endl;
      indent_up();
    }
...
void t_kotlin_generator::generate_kotlin_struct_tuple_reader(ostream &out, t_struct *tstruct) {
  indent(out) << "override fun read(iprot: org.apache.thrift.protocol.TProtocol"
              << ", struct:" << tstruct->get_name() << ") {" << endl;
  indent_up();
...
void t_kotlin_generator::generate_kotlin_struct_tuple_writer(ostream &out, t_struct *tstruct) {
  indent(out) << "override fun write(oprot:org.apache.thrift.protocol.TProtocol"
              << ", struct:" << tstruct->get_name() << ") {" << endl;
  indent_up();
  indent(out) << "val prot:org.apache.thrift.protocol.TTupleProtocol = "
